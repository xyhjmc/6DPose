...
class C2SDWR(nn.Module):
    """
    C2f + DWR 串联 + Shift_Module 复合模块
    结构：
      x
      └─ cv1 → [2*h]
          └─ split ┬─ y1 ─ DWR* n ─ shift ─┐
                    └─ y2 ─――――――――――――――――┘
          └─ concat(shifted y1, y2) → cv2 → out
    """
    """
    C2SDWR Module Structure:

        Input x [B, c1, H, W]
          │
          ▼
        cv1: 1×1 Conv (c1 → 2·h)             # now [B, 2h, H, W]
          │
          ▼
        split → y1 [B, h, H, W]    y2 [B, h, H, W]
          │                          │
          │                          └─ identity ──┐
          │                                       │
          ▼                                       │
    ...
    def __init__(self, c1, c2, n=1, shift_size=1):
        super().__init__()
        hidden = int(c2 * 0.5)  # 原 C2f 的中间通道
        # 1×1 降维到 2*hidden
        self.cv1   = Conv(c1, 2 * hidden, k=1, s=1)
        # 串联 n 个 DWR，作用在 y1 分支
